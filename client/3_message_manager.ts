...
export class MessageManager implements UpdateProcessor<MessageManagerUpdate, true> {
  ...
  static #CAPTIONABLE_MESSAGE_TYPES = ["photo", "document", "video", "animation", "voice", "audio", "video", "livePhoto"] as const;
  ...
  async #uploadPhoto(peer: Api.InputPeer | undefined, photo: FileSource, params?: SendPhotoParams & { video?: FileSource }) {
    ...
    if (media === null) {
      if (!video && typeof photo === "string" && isHttpUrl(photo)) {
  ...
  async reportEphemeralMessage(chatId: ID, messageId: number, params?: ReportEphemeralMessageParams): Promise<ReportResult> {
    this.#c.storage.assertUser("reportEphemeralMessage");
    const peer = await this.#c.getInputPeer(chatId);
    const id = messageId;
    const option = params?.option ? encodeText(params.option) : new Uint8Array();
    const message = params?.text ?? "";
    const result = await this.#c.invoke({ _: "ephemeral.reportMessage", peer, id, option, message });
